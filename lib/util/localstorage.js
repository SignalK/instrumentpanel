...
export function generateBackupSettings() {
  ...
  backupSettings[enPathSubscription] = getStoredData(enPathSubscription);
...
export function storeAllSettings(settingsData) { // settingsData typeof object
  try {
    ...
    if (typeof settingsData === 'object') {
      ...
      if (settingsData[enPathSubscription]) {
        window.localStorage.setItem(enPathSubscription, settingsData[enPathSubscription]);
      }
      return true;
    }
  } catch (ex) {
