...
export default class IpNavBar extends React.Component {
  ...
  disconnectedButtons() {
    const disableButton = (this.props.model.get().helpVisible) ? true : false;
    const skClientOptions = this.props.instrumentPanel.SignalkClient.options;
    ...
    return (
      <span>
        <Button onClick={this.connectOrDisconnect} style={{margin:5}} disabled={disableButton}>
          ...
          {skClientOptions.useTLS ? 's://' : '://'}
          {skClientOptions.hostname}:{skClientOptions.port}
        </Button>
        <div className="btn-group" role="group" style={btnGroupStyle}>
          <Button className={buttonClass + (disableButton ? ' btn-warning' : '')} style={buttonStyle}
            onClick={this.toggleHelpVisible}>
  ...
  componentDidMount() {
    ...
    if (window.localStorage['signalKHostConnected'] === 'true' &&
        window.localStorage['signalKLastHost'] === skClientOptions.hostname + ':' + skClientOptions.port) {
  ...
  connectOrDisconnect() {
    ...
    if (!(this.state.connecting || this.state.connected)) {
      ...
      SignalkClient.on('connect', () => {
        SignalkClient.unsubscribe(); // to fix when autoConnect = true
      ...
      SignalkClient.on('hitMaxRetries', () => {
        ...
        alert("Cannot connect to signal k server: ws" +
          ((SignalkClient.options.useTLS) ? "s" : "") +
          "://" + SignalkClient.options.hostname +
          ":" + SignalkClient.options.port);
      });
    } else {
        try {
