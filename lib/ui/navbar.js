...
export default class IpNavBar extends React.Component {
  ...
  hideIpNavBar() {
    ...
    this.setState({ visible: false },
      () => {
        ...
        this.props.instrumentPanel.navBarVisible.set(false);
        if (this.props.instrumentPanel.enPathSubscription) {
          let subscribes = {
            context: "vessels.self",
            subscribe: []
          }
          if (this.props.instrumentPanel.currentPage !== notificationPageId) {
            this.props.instrumentPanel.getEnabledWidgets().forEach(widget => {
              if (typeof widget.options.activeSources != 'undefined') { // windmeter specific
                Object.values(widget.options.activeSources).forEach(value => {
                  subscribes.subscribe.push(pathToSubscribe(value.path))
                })
              } else {
                subscribes.subscribe.push(pathToSubscribe(widget.options.path))
              }
            });
          }
          subscribes.subscribe.push(pathToSubscribe("notifications.*"))
          this.props.instrumentPanel.SignalkClient.send('{"context":"*","unsubscribe":[{"path":"*"}]}')
          this.props.instrumentPanel.SignalkClient.send(subscribes)
        }
      }
    );
  }
  ...
  showIpNavBar(event) {
    ...
    this.setState({ visible: true },
      () => {
        this.props.instrumentPanel.navBarVisible.set(true);
        if (
          this.props.model.get().settingsVisible ||
          !this.props.model.get().isLocked ||
          this.props.model.get().helpVisible ||
        ...
        this.hideNavBarTimer = setTimeout(() => (this.hideIpNavBar()), 1500000);
        if (this.props.instrumentPanel.enPathSubscription) {
          this.props.instrumentPanel.SignalkClient.send('{"context":"*","unsubscribe":[{"path":"*"}]}')
          this.props.instrumentPanel.SignalkClient.send('{"context":"vessels.self","subscribe":[{"path":"*","policy":"instant","format":"delta","minPeriod":"500"}]}')
        }
      }
    );
  }
  ...
  renderHide() {
    ...
    return (
      <Navbar fixed="top">
        <Navbar.Collapse className="justify-content-end">
          {notificationIcon}
          <ConnectionIndicator size="1em" allUpdates={this.props.instrumentPanel.streamBundle.pathValues} />
        </Navbar.Collapse>
        &nbsp;
      </Navbar>
    )
  ...
  connectedButtons() {
    ...
    var pageButton = function (page, i) {
      return (
        <Button variant="ip"
          key={i}
          active={this.props.instrumentPanel.currentPage === i}
    ...
    var notificationButton = (
      <Button variant="ip"
        key={notificationPageId}
        active={this.props.instrumentPanel.currentPage === notificationPageId}
        onClick={(event) => { this.setPage(notificationPageId, event) }}
        onMouseDown={this.handleStopPropagation}
        disabled={disableButton}
      >
        <SvgNotification24px x="0" y="0" width="1.5em" height="1.5em" style={notificationIconStyle} />
      </Button>
    );
    ...
    var settingIcon = this.props.model.get().settingsVisible ? (
      <SvgVisibility24px x="0" y="0" width="1.5em" height="1.5em" />) : (
      <SvgBuild24px x="0" y="0" width="1.5em" height="1.5em" />);

    var lockIcon = this.props.model.get().isLocked ? (
      <SvgLock24px x="0" y="0" width="1.5em" height="1.5em" />) : (
      <SvgLockOpen24px x="0" y="0" width="1.5em" height="1.5em" />);

    var darkModeIcon = (this.props.instrumentPanel.getDarkMode()) ? (
      <SvgSunny24px x="0" y="0" width="1.5em" height="1.5em" />) : (
      <SvgDark24px x="0" y="0" width="1.5em" height="1.5em" />);

    var fullscreenIcon = (this.state.fullscreen) ? (
      <SvgCloseFullscreen24px x="0" y="0" width="1.5em" height="1.5em" />) : (
      <SvgOpenInFull24px x="0" y="0" width="1.5em" height="1.5em" />);
    ...
    return (
      <React.Fragment>
        <Button variant="ip"
          ...
          onMouseDown={this.handleStopPropagation}
          active={!this.props.model.get().isLocked}
          disabled={(
            disableButton ||
        ...
        <Button variant="ip"
          ...
          onMouseDown={this.handleStopPropagation}
        >
          <SvgHelpOutline24px x="0" y="0" width="1.5em" height="1.5em" />
        </Button>
        &nbsp;
        <Button variant="ip"
        ...
        &nbsp;
        {(this.standalone) ? null : <Button variant="ip"
