...
var IpNavBar = React.createClass({
  ...
  doConnectOrDisconnect: function(hostSpec) {
    if (hostSpec) {
      if (!(this.state.connecting || this.state.connected)) {
        var thisOnConnect = this.onConnect;
        const splitHostSpec = hostSpec.split(':')
        const hostname = splitHostSpec[0]
        const port = splitHostSpec.length > 1 ? splitHostSpec[1] : 80
        const that = this
        this.setState({
          connecting: true,
          host: hostSpec,
          hosts: this.addToHosts(hostSpec, this.state.hosts)
        });
        signalk.connectP(
          {
            hostname: hostname,
            port: port,
            onData: this.props.instrumentPanel.dispatch.bind(this.props.instrumentPanel),
            onDisconnect: that.onDisconnect,
            onError: function(error) {
              console.log(error)
              that.setState({connecting: false, connected:false})
            },
            subscribeParameter: 'self'
          }
        ).then((connection) => {
          that.onConnect.call(that, hostSpec, connection)
        }).catch((error) => {
          console.log(error)
          that.setState({connecting: false, connected:false}, )
        })
        try {
          window.localStorage['signalKLastHost'] = hostSpec;
  ...
  onConnect: function(host, connection) {
    this.setState({
      connecting: false,
      connected: true,
