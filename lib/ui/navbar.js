...
export default class IpNavBar extends React.Component {
    ...
    this.setState({ visible: false },
      () => {
        ...
        if (this.props.instrumentPanel.enPathSubscription) {
          ...
          debug("[hideIpNavBar] subscribe to:" + JSON.stringify(subscribes))
  ...
  showIpNavBar(event) {
    ...
    this.setState({ visible: true },
      () => {
        ...
        if (this.props.instrumentPanel.enPathSubscription) {
          this.props.instrumentPanel.SignalkClient.send('{"context":"*","unsubscribe":[{"path":"*"}]}')
          var subscribeCmd = '{"context":"vessels.self","subscribe":[{"path":"*","policy":"instant","format":"delta","minPeriod":"500"}]}';
          debug("[showIpNavBar] subscribe to:" + subscribeCmd);
  ...
  renderHide() {
    ...
    const notificationIcon = ((this.props.instrumentPanel.notificationLevel > 1) ||
      (this.props.instrumentPanel.notificationLevelHidden > 1) ||
      (this.props.instrumentPanel.notificationLevelMax > 1)) ?
      <SvgNotification24px
        x="0"
        y="0"
        width="1em"
        height="1em"
        fill={this.props.instrumentPanel.notificationColorBell}
        style={borderStyle}
      /> : null;
    return (
