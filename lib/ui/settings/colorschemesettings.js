...
export default class ColorSchemeSettings extends React.Component {
  ...
  render() {
    var menuItems = Object.keys(menuSettings).map(key => {
      return <Dropdown.Item key={key} eventKey={key} active={this.state.colorSchemeSetBy === key}>{menuSettings[key].title}</Dropdown.Item>
    });
    var OsSupportText = (
    ...
    return (
      <div className="colorSchemeSettings">
        ...
        >
          <Dropdown.Toggle>
            {menuSettings[this.state.colorSchemeSetBy].title}
          </Dropdown.Toggle>
