...
export default class SaveLoadSettingsUnits extends React.Component {
  ...
  handleFileLoad(event) {
    ...
    const errorMessage = "Loading settings from file failed !";
    var reader = new FileReader();
    if ((typeof filenameSettings !== 'undefined') && (filenameSettings.name !== '')) {
      reader.onload = (event) => {
        this.setState({ readOnlyButton: true });
        try {
          var settingsData = JSON.parse(event.target.result);
          if (!lsStoreAllSettings(settingsData)) { throw errorMessage }
    ...
    ssStoreHostLayouts(
      ...
      (opRet => {
        ...
        if (opRet === true) {
          ...
          this.setMessageProgress("Saving layout on server failed !", true);
        }
        this.setState({ readOnlyButton: false });
      })
    )
    ...
    ssRetrieveLayoutsData(
      ...
      ((date, data, isUser) => {
        ...
        try {
          ...
          console.error(ex);
          this.setMessageProgress("Loading layout from server failed !");
          this.setState({ readOnlyButton: false });
        }
      })
    )
    ...
    ssDeleteHostLayouts(
      ...
      (opRet => {
        ...
        if (opRet === true) {
          ...
          this.setMessageProgress("Deleting layout on server failed !", true);
        }
        this.setState({ readOnlyButton: false });
      })
    )
  ...
  handleSsPrefUnitsSave() {
    ...
    ssStorePreferredUnits(
      ...
      (opRet => {
        ...
        if (opRet !== true) {
          this.setMessageProgress("Saving preferred units on server failed !", true);
        } else {
    ...
    ssRetrievePreferredUnits(
      ...
      (preferredUnits => {
        ...
        if (preferredUnits !== null) {
          ...
          this.setMessageProgress("Loading preferred units from server failed !");
        }
        this.setState({ readOnlyButton: false });
      })
    )
  ...
  renderFileSave() {
    return (
      <Container key="fileSave">
        <Button variant="ip" onClick={this.handleFileSave}>Save all settings</Button>
        &nbsp;to local file
      </Container >
    );
  }
  ...
  renderSsLayoutsLoad() {
    ...
    var allLayoutsList =
      Object.keys(this.state.ssLayouts).map(function (infosKey, i) {
        ...
        infoDetails = (that.state.ssLayoutLoadKey === infosKey) ? (
    ...
    if (Object.keys(this.state.ssLayouts).length === 0) {
      return (
        <Container key="ssLoad" className="noItem">No layout found on the server</Container >
      )
    }
    return (
      <Container key="ssLoad" className="noItem layoutLoad" >
  ...
  renderSsPrefUnitsLoad() {
    ...
    return (
      <Container key="ssPrefUnitsLoad" className="noItem">
    ...
    return (
      <React.Fragment>
        ...
        >
          ...
          />
          <Form.Check.Label>{actionTxt} this user <b> {this.state.username}</b></Form.Check.Label>
        </Form.Check>
        <Form.Check
          key="2"
          type='radio'
          checked={!this.state.ssUser}
          onChange={this.handleSsUserOrGlobalChange}
          label={`${actionTxt} global settings`}
        />
      </React.Fragment>
    )
  }
    ...
    if (this.props.displayKey === saveDisplayKey) {
      return (
        <div className="saveload">
          <Container key="saveToServer" className="noItem">
