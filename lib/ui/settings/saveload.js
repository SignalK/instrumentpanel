...
export default class SaveLoadSettingsUnits extends React.Component {
  ...
  renderIsLoggedAndSsAvaliable() {
    ...
    if (this.state.readWriteLevel) {
      return (
        <Alert bsStyle="info">
          You are logged in as <b>{this.state.username}</b> with <b>read/write</b> acces. To save in global settings, you must <a href={AdminUrl}>log in</a> with <b>admin</b> acces.
        </Alert >
      )
    }
    if (this.state.readOnlyLevel) {
      return (
        <Alert bsStyle="info">
          You are logged in as <b>{this.state.username}</b> with <b>read only</b> acces. To save on server, you must <a href={AdminUrl}>log in</a> with <b>read/write</b> or <b>admin</b> acces.
        </Alert >
      )
    }
    if (!this.state.loggedIn) {
      return (
        <Alert bsStyle="info">
          You are not logged in. To save on server, you must <a href={AdminUrl}>log in</a> with <b>read/write</b> or <b>admin</b> acces.
        </Alert >
      )
    }
  }
