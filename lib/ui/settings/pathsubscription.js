...
export default class PathSubscription extends React.Component {
  ...
  render() {
    return (
      <Container>
        <Form.Check type='checkbox' onChange={this.handleChange} label='Path Subscription' checked={this.state.enable} />
        <Form.Text>
          By default, InstrumentPanel subscribes to all paths below 'self' (see 'Data Browser' in the SignalK server).<br />
