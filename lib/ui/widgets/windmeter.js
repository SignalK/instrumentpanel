...
var subscribes = {
  'environment.wind.angleApparent': function(sourceId, stream) {
    if (!this.state.activeStreams['environment.wind.angleApparent']) {
      var that = this;
      this.state.activeStreams['environment.wind.angleApparent'] = stream.onValue(function(pathValue) {
        that.setState({
  ...
  'environment.wind.speedApparent': function(sourceId, stream) {
    var that = this;
    if (!this.state.activeStreams['environment.wind.speedApparent']) {
      this.state.activeStreams['environment.wind.speedApparent'] = stream.onValue(function(pathValue) {
        that.setState({
          speed: pathValue.value
        })
      })
    }
  },
...
var componentClass = React.createClass({
  ...
  componentDidMount: function() {
    var that = this;
    this.props.streamBundle.newSources.onValue(function(newStream){
      if (subscribes[newStream.path]) {
        subscribes[newStream.path].call(that, newStream.sourceId, newStream.stream)
      }
    });
  },
...
function WindMeter(id, options, streamBundle, instrumentPanel) {
  console.log("windmeter " + options.path + " " + id)
