...
function WindMeter(id, options, streamBundle, instrumentPanel) {
  ...
  this.valueStream = this.valueStreamBus.toProperty({});
  this.combineValueStream(this.options.displayModeIndex);
  this.aliveStatusStreamBus = new Bus();
  this.optionsBundle.aliveStatusStream = this.aliveStatusStreamBus.toProperty();
...
WindMeter.prototype.handleNewSource = function(newSource) {
  if(pathsCovered.includes(newSource.path)) {
    if (!this.availableSources.includes(newSource.key)) {
      ...
      this.instrumentPanel.schemadata.getAsyncMetaForVesselPath(newSource.path, this.instrumentPanel.metaUpdateInWS)
      .then(meta => {
        this.streamBundle.metasStream.push({path: newSource.path, value: meta});
      })
      this.combineValueStream(this.options.displayModeIndex);
    }
    return true;
...
WindMeter.prototype.combineValueStream = function(displayModeIndex) {
  ...
  if(angleStream && speedStream) {
    ...
    this.unplugs.push(this.aliveStatusStreamBus.plug(
      combineWith(
        this.streamBundle.getAliveStatusStreamForSourcePath(
          angleSource.sourceId,
          angleSource.path,
          this.instrumentPanel.schemadata.getSyncMetaForVesselPath),
        this.streamBundle.getAliveStatusStreamForSourcePath(
          speedSource.sourceId,
          speedSource.path,
          this.instrumentPanel.schemadata.getSyncMetaForVesselPath),
        (AliveStatusAngle, AliveStatusSpeed) => {
          return ({
            isValid: (AliveStatusAngle===0 || AliveStatusSpeed===0) ? 0 : (AliveStatusAngle===1 || AliveStatusSpeed===1) ? 1 : -1,
            streams: {
              [angleSource.path]: AliveStatusAngle,
              [speedSource.path]: AliveStatusSpeed
            }
          })
        }
      )
    ...
    this.unplugs.push(this.aliveStatusStreamBus.plug(
      this.streamBundle.getAliveStatusStreamForSourcePath(
        ...
        angleSource.path,
        this.instrumentPanel.schemadata.getSyncMetaForVesselPath)
      .map((AliveStatusAngle) => {
        return ({
          isValid: AliveStatusAngle,
          streams: {
            [angleSource.path]: AliveStatusAngle,
            [speedSource.path]: -2
          }
    ...
    this.unplugs.push(this.aliveStatusStreamBus.plug(
      this.streamBundle.getAliveStatusStreamForSourcePath(
        ...
        angleSource.path,
        this.instrumentPanel.schemadata.getSyncMetaForVesselPath)
      .map((AliveStatusSpeed) => {
        return ({
          isValid: AliveStatusSpeed,
          streams: {
            [angleSource.path]: -2,
            [speedSource.path]: AliveStatusSpeed
          }
