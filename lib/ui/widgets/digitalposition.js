    ...
    render: function() {
      return (
        <svg key={id} height="100%" width="100%" viewBox="0 0 20 40">
          <text x="10" y="19" textAnchor="middle" fontSize="16" dominant-baseline="middle">{(this.state.longitude ? this.state.longitude.toFixed(3) : '--.--')+'\u00b0'}</text>
          <text x="10" y="35" textAnchor="middle" fontSize="16" dominant-baseline="middle">{(this.state.latitude ? this.state.latitude.toFixed(3) : '--.--')+'\u00b0'}</text>
