...
export function getPositionFormatLabels() {
  return positionFormats.map(positionFormat => { return positionFormat({ latitude: 0, longitude: 0 }).label })
}

export function getIndexFromPositionFormatLabel(label) {
  return positionFormats.findIndex(positionFormat => { return positionFormat({ latitude: 0, longitude: 0 }).label === label })
}
...
function DigitalPosition(id, options, streamBundle, instrumentPanel) {
  ...
  if (typeof this.options.formatPositionIndex === 'undefined') {
    let index = getIndexFromPositionFormatLabel(instrumentPanel.preferredUnits['position']);
    this.options.formatPositionIndex = (index > 0) ? index : 0;
  }
  ...
  class ReactComponent extends React.Component {
      ...
      try {
        return (
          <svg key={id} height="100%" width="100%" viewBox="0 0 20 40" stroke="none">
            <text x="10" y={this.dynamicPositions.yLat} textAnchor="middle" fontSize={this.state.value.fontSize + this.dynamicPositions.incFontSize} dominantBaseline="middle">{this.state.value.latitude}</text>
            <text x="10" y={this.dynamicPositions.yLong} textAnchor="middle" fontSize={this.state.value.fontSize + this.dynamicPositions.incFontSize} dominantBaseline="middle">{this.state.value.longitude}</text>
            {this.dynamicPositions.labelVisible && <text x="10" y="4" textAnchor="middle" fontSize="6" dominantBaseline="middle" textDecoration={this.dynamicPositions.labelTextDecoration}>{this.widgetLabel}</text>}
          </svg>
        )
    ...
    onNavBarHideShow(navBarVisible) {
      ...
      if (this.dynamicPositions.labelVisible) {
        this.dynamicPositions.yLat = 17
        this.dynamicPositions.yLong = 33
        ...
        this.dynamicPositions.yLat = 14
        this.dynamicPositions.yLong = 31
        this.dynamicPositions.incFontSize = 5
...
DigitalPosition.prototype.getSettingsElement = function (pushCellChange) {
  return this.getSettingsElementHideLabelOnly(pushCellChange);
}
