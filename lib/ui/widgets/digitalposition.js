    ...
    render: function() {
      ...
      return (
        <svg key={id} onClick={this.changePositionFormat} height="100%" width="100%" viewBox="0 0 20 40">
          <text x="10" y="19" textAnchor="middle" fontSize={positionObject.fontSize} dominantBaseline="middle">{positionObject.latitude}</text>
          <text x="10" y="35" textAnchor="middle" fontSize={positionObject.fontSize} dominantBaseline="middle">{positionObject.longitude}</text>
          <text x="10" y="4" textAnchor="middle" fontSize="4" dominantBaseline="middle">{positionObject.label + '  ' + options.sourceId}</text>
          <FontAwesome className="clickIconWidget" name="hand-o-up" />
        </svg>
      )
    }
  }),{
