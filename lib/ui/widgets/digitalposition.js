    ...
    render: function() {
      return (
        <svg key={id} height="100%" width="100%" viewBox="0 0 20 40">
          <text x="10" y="19" textAnchor="middle" fontSize="16" dominant-baseline="middle">{getPrefixedValue(this.state.longitude, 'E', 'W') + '\u00b0'}</text>
          <text x="10" y="35" textAnchor="middle" fontSize="16" dominant-baseline="middle">{getPrefixedValue(this.state.latitude, 'N', 'S') + '\u00b0'}</text>
          <text x="10" y="4" textAnchor="middle" fontSize="5" dominant-baseline="middle">{"Position " + options.sourceId}</text>
        </svg>
      )
    }
  }),{
...
function getPrefixedValue(value, positivePrefix, negativePrefix) {
  if (value) {
    return (value >= 0 ? positivePrefix : negativePrefix) + value.toFixed(4)
  } else {
    return '--.--'
  }
}
