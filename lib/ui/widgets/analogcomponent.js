...
export default class AnalogComponent extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      statesColor: (typeof props.options.statesColor === 'object') ? props.options.statesColor : defaultStatesColor
    };
    this.widgetZones = [];
  ...
  buildWidgetZones() {
    ...
    if (this.state.zoneMode === 'local') {
      ...
      this.props.options.zones.forEach(zone =>{
        ...
        widgetZone.color = colorsToHTML[that.state.statesColor[zone.state]]
