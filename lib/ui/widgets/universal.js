...
//  this.unit = this.getUnitForPath(options.path) || "";
  const schemaUnit = schema.getUnits('vessels.foo.' + options.path);

  if (schemaUnit === undefined) {
    var widget = this;
    widget.delayed = 1;
    signalkUnits = new signalkClient(instrumentPanel.host.split(':')[0], instrumentPanel.host.split(':')[1], instrumentPanel.useSSL);
    signalkUnits.apiGet('/vessels/self/' + options.path.replace(/\./g, '/') + '/meta/units')
    .then(function (response) {
      console.log(response.req.url + ':' + response.status + ':' + response.text);
      widget.unit = JSON.parse(response.text);
      widget.createUpdateWidgets(widget);
      widget.delayed = 0;
...
Widget.prototype.createUpdateWidgets = function(widget) {
  ...
  if ((widget.options.convertTo) && (widget.unit !== '')) {
  ...
  if ((widget.delayed === 1) && (widget.widgets.length > 0)) {
    var newWidgets  = [];
    ...
    widget.widgets = newWidgets;
      ...
      this.widgets = [React.createElement(digitalComponent,{
        ...
        path: widget.options.path,
        sourceId: widget.options.sourceId,
        maxValue: widget.options.maxValue,
        maxValueStream: widget.maxValueStream.toProperty(),
        minValue: widget.options.minValue,
        minValueStream: widget.minValueStream.toProperty(),
        redLine: widget.options.redLine,
        redLineStream: widget.redLineStream.toProperty()
      })];
    }
}
