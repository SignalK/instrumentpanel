...
class ZoneModeSettings extends React.Component {
  ...
  handleInputChange(event) {
    ...
    var options = this.state.options;
    options[eventID] = event.target.value;
    this.setState({ options }, this.props.onSettingsChange(event));
  }
      ...
      <table>
        <tbody>
          <tr>
            ...
            <td>
              <select value={props.options.decimals} style={{ marginLeft: 10 }} onChange={props.onDecimalSettingsChange}>
    ...
    <fieldset>
      <table>
        <tbody>
          <tr>
            <td>
              <label htmlFor={"label" + props.id}>Label</label>
            </td>
            <td>
              <input id={"label" + props.id} style={{ marginLeft: 10 }} size="4" onChange={props.onLabelChange} defaultValue={!!(props.options.localLabel) ? props.options.localLabel : props.options.label} />
            </td>
          </tr>
        </tbody>
      </table>
...
return (
    <div>
      ...
      {props.options.selectedWidget === 1 ? undefined : labelSettings}
      {props.options.selectedWidget === 1 ? undefined : decimalSettings}
    </div>
  )
}
...
Widget.prototype.getSettingsElement = function (pushCellChange) {
  ...
  return SettingsPanel({
    ...
    id: this.id,
    getSettingsElementHideLabelOnly: that.getSettingsElementHideLabelOnly.bind(that),
    pushCellChange: pushCellChange,
    ...
    onDecimalSettingsChange: function (event) {
        that.options.decimals=event.target.value;
        that.optionsBundle.setOptions(that.options);
        pushCellChange();
    },
    onLabelChange: function (event) {
        that.options.localLabel=event.target.value;
        that.optionsBundle.setOptions(that.options);
        pushCellChange();
    }
  });
