...
function SpecificText(id, options, streamBundle, instrumentPanel) {
  ...
  class SpecificTextComponent extends React.Component {
    ...
    render() {
      const specificRender = (typeof this.props.renderFunction === 'function') ?
        this.props.renderFunction.bind(this) : defaultRender.bind(this);
      try {
        return specificRender();
      } catch (error) {
        console.error(error);
        return errorRender.bind(this)();
      }
    }
  }
...
function errorRender() {
  return (
    <svg height="100%" width="100%" viewBox="0 0 20 33" stroke="none">
      ...
      <text x="10" y="20" textAnchor="middle" fontSize="6" dominantBaseline="middle">
...
function environmentCurrent() {
  var conversion = getConversion('rad', 'deg');
  var setTrue = conversion(this.state.value.setTrue);
  var setMagnetic = conversion(this.state.value.setMagnetic);
  var drift = this.state.value.drift;
  var unit = this.props.options.unit;
  var convertTo = this.props.options.convertTo || unit;
  if (convertTo !== unit) {
    conversion = getConversion(unit, convertTo);
    if (typeof conversion === 'function') {
      unit = convertTo;
      drift = conversion(drift);
    }
  }
  return (
...
function designLength() {
  ...
  var convertTo = this.props.options.convertTo || unit;
  if (convertTo !== unit) {
    const conversion = getConversion(unit, convertTo);
    if (typeof conversion === 'function') {
      unit = convertTo;
      overall = conversion(overall);
      hull = conversion(hull);
      waterline = conversion(waterline);
    }
  }
  return (
    <svg height="100%" width="100%" viewBox={this.props.viewBox} stroke="none">
