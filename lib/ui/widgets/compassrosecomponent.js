...
export default class CompassRoseComponent extends React.Component {
  ...
  renderDirName(angle) {
    ...
    return dirs[Math.round(angle / 11.25)];
  }
    ...
    return (
      ...
      <g transform="scale(0.8) translate(50 50)">
        <g transform={centerRotate(-angle)}>
          ...
          {mainDirs}
          {minorDirs}
          {(angleText !== 'NaN') ? this.renderRoseStyle2() : undefined}
        </g>
      </g>
        <text x="200" y="24" textAnchor="middle" fontSize="20" stroke="none">{this.props.options.label}</text>
        <text x="200" y="325" textAnchor="middle" fontSize="28" stroke="none">{angleText}</text>
        <text x="200" y="390" textAnchor="middle" fontSize="20" stroke="none">{(angleText !== 'NaN') ? this.renderDirName(angle) : angleText}</text>
