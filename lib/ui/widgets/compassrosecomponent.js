  ...
  render: function() {
    ...
    return (
      ...
      <g transform="scale(0.8) translate(50 50)">
        <g stroke={textColor} fill={textColor} transform={centerRotate(-this.state.value)}>
          {this.renderTicks()}
          {mainDirs}
          {minorDirs}
        ...
        <text x="200" y="24" textAnchor="middle" fontSize="20" fill={textColor}>{this.state.label + " " + this.props.sourceId}</text>
        <text x="200" y="325" textAnchor="middle" fontSize="28" fill={textColor}>{this.state.value.toFixed(2)}</text>
        <text x="200" y="390" textAnchor="middle" fontSize="20" fill={textColor}>{this.renderDirName()}</text>
