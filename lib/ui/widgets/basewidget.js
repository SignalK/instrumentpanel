...
export default function BaseWidget(id, options, streamBundle, instrumentPanel) {
  ...
  this.optionsBundle = {
    ...
    busNewOptions: new Bus()
  }

  if (this.multiStream !== true) {
    this.valueStream = this.streamBundle.getValueStreamForSourcePath(this.options.sourceId, this.options.path);
    this.streamBundle.getMetaStreamForSourcePath(options.sourceId, options.path).onValue( meta => {
      let options = this.metaToOptions(meta);
      this.optionsBundle.setOptions(options);
    })
    this.instrumentPanel.schemadata.getAsyncMetaForVesselPath(this.options.path, this.instrumentPanel.metaUpdateInWS)
    .then(meta => {
      this.streamBundle.metasStream.push({path: this.options.path, value: meta});
    })
    this.optionsBundle.aliveStatusStream = this.streamBundle.getAliveStatusStreamForSourcePath(
      this.options.sourceId,
      this.options.path,
      this.instrumentPanel.schemadata.getSyncMetaForVesselPath
    );
  }
}
...
// end compatibility with previous versions.
  ...
  once(this.options).combine(this.valueStream, (options, data) => {return([options, data.value])})
  .take(1)
  .onValue(this.optionsBundle.busNewOptions.push)
...
BaseWidget.prototype.metaToOptions = function(pathMeta) {
  ...
  pathMeta.value.timeout && (options.timeout = pathMeta.value.timeout);
...
export function defaultComponentDidMount(component, preConversion, postConversion) {
  ...
  component.unsubscribes.push(
    component.props.valueStream.onValue(
      (data => {
        component.setState({
          value: fullConversion(data.value)
        });
      }).bind(component)
    )
  );
  ...
  component.unsubscribes.push(
    component.props.optionsBundle.busNewOptions.onValue( ([options, value]) => {
      component.conversion = component.props.optionsBundle.getConversionForOptions();
