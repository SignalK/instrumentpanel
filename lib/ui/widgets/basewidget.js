...
export default function BaseWidget(id, options, streamBundle, instrumentPanel) {
  ...
  this.valueStream = this.streamBundle.getValueStreamForSourcePath(options.sourceId, options.path).map((timestampedValue) => {return timestampedValue.value});
  this.timestampStream = this.streamBundle.getValueStreamForSourcePath(options.sourceId, options.path).map((timestampedValue) => {return timestampedValue.timestamp});;
  ...
  this.optionsBundle.getIsValidStream = combineWith(this.timestampStream, this.streamBundle.checkTimeout, this.instrumentPanel.navbarVisible, (timestamp, checkTimeout, navbarVisible) => {
  if (!this.options.timeout) return [-1, navbarVisible];
    let tmpNow = Date.now()
    let result = tmpNow - timestamp < this.options.timeout*1000
//    console.log(this.options.timeout)
    if (!result) console.log(result,this.options.key, tmpNow, timestamp, (tmpNow - timestamp), this.options.timeout*1000)
  return result ? [1, navbarVisible] : [0, navbarVisible];
  })
  .skipDuplicates() //comment this to see all the checks
//  .log('isValid')
