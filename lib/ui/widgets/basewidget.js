    ...
    if (newUnit !== '') {
      ...
      if (widget.options.convertTo) {
        ...
        if ((conversions !== undefined) && (conversions[widget.options.convertTo])) {
          ...
          console.error("No such conversion:" + newUnit + "=>" + widget.options.convertTo)
        }
      }
    } else {
        console.warn('No such units for '+widget.options.path);
      }
    if ((newUnit !== widget.unit) || (newLabel !== widget.options.path)) {
      widget.unit = newUnit || widget.unit;
      widget.options.path = newLabel || widget.options.path;
      if ((widget.updateFromOnlineMeta === 1) &&
           (widget.widgets.length > 0)) {
 //       console.log('updateFromOnlineMeta:update => unit:'+widget.unit+':label'+widget.options.path+':path:'+widget.options.path);
        var newWidgets  = [];
