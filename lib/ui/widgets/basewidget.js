...
export default function BaseWidget(id, options, streamBundle, instrumentPanel) {
  ...
  this.optionsBundle.getIsValidStream = combineWith(this.timestampStream, this.streamBundle.checkTimeout, (timestamp, checkTimeout) => {
    if (!this.options.timeout) return true;
    let tmpNow = Date.now()
