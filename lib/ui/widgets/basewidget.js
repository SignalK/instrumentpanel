...
BaseWidget.prototype.getSettingsElementHideLabelOnly = function (pushCellChange) {
  var that = this;
  return settingsPanelHideLabelOnly({
    options: this.options,
    onHideLabelChange: function (event) {
      that.options.hideLabel = !that.options.hideLabel;
      that.optionsBundle.setOptions(that.options);
      that.instrumentPanel.navBarVisible.set(!that.instrumentPanel.navBarVisible.get());
      that.instrumentPanel.navBarVisible.set(!that.instrumentPanel.navBarVisible.get());
      pushCellChange();
    }
  });
...
BaseWidget.prototype.setOptions = function (options) {
  if (options !== null) {
    ...
    */
    ...
    if (!isEqual(optionsOrg, this.options)) {
...
BaseWidget.prototype.getFixedDecimalByUnit = function (unit) {
  return (unit === 'inHg') ? 2 : undefined;
}

BaseWidget.prototype.defaultPostConversion = function (value, fixedDecimal) {
  if (typeof value === 'number') {
    if (isNaN(value)) return '-';
    if (typeof fixedDecimal !== 'number') {
...
BaseWidget.prototype.metaToOptions = function (pathMeta) {
  ...
  let label = pathMeta.value.displayName || pathMeta.value.shortName || pathMeta.value.longName;
...
export function defaultComponentDidMount(component, preConversion, postConversion) {
  ...
  function fullConversion(value) {
    ...
    if (typeof postConversion === 'function') {
      value = postConversion(value, component.fixedDecimal);
    }
    return value;
  }
  ...
  component.onNavBarHideShow && component.unsubscribes.push(
    component.props.instrumentPanel.navBarVisible.onValue((navBarVisible) => {
      component.onNavBarHideShow(navBarVisible);
      component.props.optionsBundle.setOptions(null);
    })
  );
