...
export default function BaseWidget(id, options, streamBundle, instrumentPanel) {
  ...
  this.metaUpdateOptions = {setUnit: true, setStream: true, setLabel: true};
...
BaseWidget.prototype.updateMetaData = function() {
  const widget= this;
  const orgOptions = JSON.stringify(widget.options);
  widget.instrumentPanel.schemadata.getMetaForVesselPath(widget.options.path, false)
  .then(meta => {
/*
...
// end compatibility with previous versions.
    ...
    if ((widget.options.unit !== '') && (this.metaUpdateOptions.setUnit !== false) && (this.metaUpdateOptions.setStream !== false) ) {
