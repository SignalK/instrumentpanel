...
export default function InstrumentPanel(streamBundle) {
  this.backupSettings = checkIfBackupSettings();
...
InstrumentPanel.prototype.pushGridChanges = function () {
  debug("[pushGridChanges] received");
...
/*
levelChanged === true force render of reactGridLayout when a widget have level changed
to update widget border
*/
InstrumentPanel.prototype.updateNotificationLevel = function (levelChanged = false) {
  ...
  this.notificationColorBell = this.notificationColors[newLevelEnabled];
  this.notificationLevelMax = newLevelEnabled;
  ...
  if (mustPushGridChanges || (levelChanged && this.currentPage === notificationPageId)) {
    this.pushGridChanges();
  }
}
