...
export default function InstrumentPanel(streamBundle) {
  ...
  this.streamBundle.metaValues.onValue(metaValue => {
    debug("[metaValues] update: " + JSON.stringify(metaValue));
    this.schemadata.addMetaInCache(metaValue.path, metaValue.value);
    this.pages.forEach(page => {
      page.widgets.filter(widget => widget.options.path === metaValue.path)
      .forEach(widget => {widget.updateMetaData()});
    });
  });
...
InstrumentPanel.prototype.getMetaUpdateAvailable = function() {
  var connectionInfo = this.SignalkClient.connectionInfo;
