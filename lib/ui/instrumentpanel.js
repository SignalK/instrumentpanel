...
import {
  ...
  retrieveWidgetActiveMode as localstorageRetrieveWidgetActiveMode,
  saveWidgetActiveMode as localstorageSaveWidgetActiveMode
...
InstrumentPanel.prototype.setFlushCacheOnReload = function() {
  this.flushCacheOnReload = true;
  ...
  this.pages.forEach(function(page) { // scan all pages to find existing widget
    page.widgets.forEach(function(existingWidget) {
      ...
      if (! foundKey) {
        ...
        newOptions.active = getWidgetActiveModeByPath(newOptions.path);
...
InstrumentPanel.prototype.loadWidgetActiveMode = function() {
  this.widgetActiveMode = localstorageRetrieveWidgetActiveMode();
  this.saveWidgetActiveMode();
}

InstrumentPanel.prototype.saveWidgetActiveMode = function() {
  localstorageSaveWidgetActiveMode(this.widgetActiveMode);
}
...
InstrumentPanel.prototype.getReloadParams = function() {
  ...
  if (this.flushCacheOnReload === true) params['flushCache'] = 'true';
  return params;
}
