...
export default function InstrumentPanel(streamBundle) {
  ...
  var port = parseInt(location.host.split(':')[1]);
  if (!Number.isInteger(port)) {
    port = (useTLS) ? 443 : 80;
  }
  const signalkServer = browser.getUrlParameter("signalkServer");
...
InstrumentPanel.prototype.persist = function() {
  if (this.currentPage !== notificationPageId) {
    ...
    this.handleSaveLayoutDelay = setTimeout(() => {
      ...
      browser.saveGrid(this.SignalkClient.get('hostname') + ':' + this.SignalkClient.get('port'), that.serializableSettings());
    }, 1000);
  }
}
...
InstrumentPanel.prototype.connected = function() {
  this.schemadata = new SignalKSchemaData({
    ...
    ssl: this.SignalkClient.get('useSSL')
  });
  if (browser.getUrlParameter("flushCache")) {
    this.flushCache = true;
  }
  if (browser.getUrlParameter("useGridFromServer")) {
    browser.ajax("layouts/" + this.SignalkClient.get('hostname') + ".json",
      this.setWidgetData.bind(this),
      function(error) {
        console.log(error)
      });
  } else {
    this.setWidgetData(browser.retrieveGrid(this.SignalkClient.get('hostname') + ':' + this.SignalkClient.get('port')));
  }
}
...
InstrumentPanel.prototype.loadPreferredUnits = function loadPreferredUnits() {
  var savedPreferredUnitsListJSON = localStorage.getItem('instrumentpanelPreferredUnits');
  var savedPreferredUnitsList = savedPreferredUnitsListJSON && JSON.parse(savedPreferredUnitsListJSON);
  if (savedPreferredUnitsList === null) {return;}

  getConversionsList().map( unitSrc => {
    const unitDst = savedPreferredUnitsList[unitSrc];
    if ((typeof unitDst !== 'undefined') &&
        (typeof getConversionsForUnit(unitSrc)[unitDst] === 'function')) {
      this.preferredUnits[unitSrc] = unitDst;
    }
  })
...
InstrumentPanel.prototype.savePreferredUnits = function savePreferredUnits() {
  localStorage.setItem('instrumentpanelPreferredUnits', JSON.stringify(this.preferredUnits));
}
...
InstrumentPanel.prototype.setPreferredUnit = function setPreferredUnit(unitSrc, unitDst) {
  const conversionsForUnit = getConversionsForUnit(unitSrc);
  if (typeof conversionsForUnit !== 'undefined') {
    if (unitSrc === unitDst) {
      this.preferredUnits[unitSrc] = undefined;
      this.savePreferredUnits();
      return true;
    }
    if (typeof conversionsForUnit[unitDst] === 'function') {
      this.preferredUnits[unitSrc] = unitDst;
      this.savePreferredUnits();
      return true;
    }
