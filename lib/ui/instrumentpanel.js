...
InstrumentPanel.prototype.pushGridChanges = function () {
  ...
  //var layout = this.getLayout(this.currentPage).map(item => ({ ...item }));
...
InstrumentPanel.prototype.sortNotificationLayout = function () {
  ...
  try {
    ...
    //this.notificationPage.layout = JSON.parse(JSON.stringify(newLayout))
    //this.notificationPage.layout = newLayout.map((item) => { return { ...item } })
