...
InstrumentPanel.prototype.persist = function() {
  const skClientOptions = this.SignalkClient.options;
  if (this.currentPage !== notificationPageId) {
    var that = this;
    ...
    this.handleSaveLayoutDelay = setTimeout(() => {
      ...
      browser.saveGrid(skClientOptions.hostname + ':' + skClientOptions.port, that.serializableSettings());
    }, 1000);
  }
}
...
InstrumentPanel.prototype.connected = function() {
  this.schemadata = new SignalKSchemaData({
    host: this.SignalkClient.options.hostname,
    port: this.SignalkClient.options.port,
    ssl: this.SignalkClient.options.useSSL
  });
  if (browser.getUrlParameter("flushCache")) {
    this.flushCache = true;
  }
  if (browser.getUrlParameter("useGridFromServer")) {
    browser.ajax("layouts/" + this.SignalkClient.options.hostname + ".json",
      this.setWidgetData.bind(this),
      function(error) {
        console.log(error)
      });
  } else {
    this.setWidgetData(browser.retrieveGrid(this.SignalkClient.options.hostname + ':' + this.SignalkClient.options.port));
  }
}
