    ...
    delta.updates.forEach(function(update) {
      ...
      update.values.forEach(function(pathValue) {
        if (signalkMainPaths[pathValue.path.split('.')[0]]) {
          that.streamBundle.push.call(that.streamBundle, sourceId, pathValue)
        }
      });
    });
  }
...
InstrumentPanel.prototype.handlePossiblyNewSource = function(newSource) {
  var sourceId = newSource.sourceId;
  var key = signalkSchema.keyForSourceIdPath(sourceId, newSource.path);
