...
function InstrumentPanel(streamBundle) {
  ...
  this.signalk = new signalkClient;
...
InstrumentPanel.prototype.handlePossiblyNewSource = function(newSource) {
  ...
  if (!handled) {
    ...
    this.signalk.getMeta('/vessels/self', pathValue.path)
      .then(widget.setMeta.bind(widget))
      .catch(function(err) {});
    widget.getHandledPaths().forEach(function(path) {
      this.widgetsByPath[path] = widget;
    }, this);
    this.widgetsBySourcePath[newSource.key] = widget;
...
InstrumentPanel.prototype.connected = function(host) {
  var that = this;
  this.signalk.getSelfMatcher(host)
