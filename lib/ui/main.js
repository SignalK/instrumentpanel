...
import {LedComponent} from '../svg/SvgLED24px';
import {
  retrieveColorScheme,
...
class App extends React.Component {
  constructor(props) {
    ...
    this.generateLedElement = this.generateLedElement.bind(this);
    ...
    instrumentPanel.navbarVisible = new Bus();
  }
...
// https://github.com/STRML/react-grid-layout/issues/382#issuecomment-299734450
  ...
  wrapWidget(widget) {
    ...
    return React.createElement(
      ...
      this.generateLedElement(widget)
    );
  }
  ...
  generateLedElement(widget) {
    if (!this.state.isLocked || !widget.reactWidget.props.optionsBundle.getIsValidStream) { return null }
    return React.createElement(
      LedComponent,
      {
        size: 24,
        isValid: widget.reactWidget.props.optionsBundle.getIsValidStream
      }
    );
  }
