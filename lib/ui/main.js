  ...
  generateToolTipsElement(widget) {
    ...
    class ToolTipsComponent extends React.Component {
      constructor(props) {
        super(props);
        this.unsubscribes = [];
        this.notificationType = typeof widget.reactWidget.props.notification !== 'undefined'
        this.state = {
      ...
      renderLed(path) {
        let isValid = (path && this.state.streams) ? this.state.streams[path] : this.state.isValid;
      ...
      renderShow() {
        let sourceToPaths = widget.handledSources().map( (source, index) => {
          ...
          return (<div key={index}>{this.renderLed(source.path)} {sourceToPath}</div>)
        })
        return (
          <div 
