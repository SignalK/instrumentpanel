...
var App = React.createClass({
  ...
  render: function() {
    ...
    return (
      <div>
      <IpNavBar model={model} dispatch={instrumentPanel.dispatch.bind(instrumentPanel)} allUpdates={streamBundle.pathValues}></IpNavBar>
      {this.state.settingsVisible ? <SettingsPanel instrumentPanel={instrumentPanel}></SettingsPanel> : ""}
      <ReactGridLayout 
      className="layout" 
      layout={this.props.widgetlayout}  
      children={this.props.widgetsModel.get().map(wrapInDiv)}
      cols={12}
      rowHeight={50}
...
var widgetsModel = Bacon.Model(instrumentPanel.getWidgets());
...
instrumentPanel.changes.onValue(function() {
  console.log("setting model")
  widgetsModel.set(instrumentPanel.getWidgets());
  app.forceUpdate();
});
