...
StreamBundle.prototype.getStatusTimeoutStreamForSourcePath = function(sourceId, path) {
  let dataStreamForSourcePath = this.checkTimeout.withLatestFrom(this.getValueStreamForSourcePath(sourceId, path), (checkTimeout, data) => {return data});
  let metaStreamForSourcePath = this.getMetaStreamForSourcePath(sourceId, path);

  return combineWith(metaStreamForSourcePath, dataStreamForSourcePath, this.navbarVisible, (pathMeta, data, navbarVisible) => {
    let timeout = pathMeta.value.timeout;
/*
0: noLed
1: emptyLed
2: blueLed
