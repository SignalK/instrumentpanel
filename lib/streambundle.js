...
var customConversions = {
  "navigation.speedThroughWater": Qty.swiftConverter('m/s', 'knots'),
  "navigation.speedOverGround": Qty.swiftConverter('m/s', 'knots')
}
...
StreamBundle.prototype.getStream = function(path) {
  ...
  if (!result) {
    ...
    if (customConversions[path]) {
      result = this.streams[path] = this.streams[path].map(customConversions[path])
    } else {
